...
if __name__ == '__main__':
    ...
    test_data_loader = torch.load('testset.pth')
    #load my model from train.py
    mymodel = torch.load('myoptimized_model.pth')
    #set my new model equal to train.py model
    testmodel.load_state_dict(mymodel)
    #set model in eval mode
    testmodel.eval()
    print("my optimized model is loaded successfully\n starting  test_data_loader")
    ...
    with torch.no_grad():
        for data,(images,labels) in enumerate(test_data_loader):
            ...
            # writer.add_scalar('Loss', test_loss.item(), len(test_data_loader)  + data)
            # writer.add_scalar('Testing Accuracy(%)', test_acc, len(test_data_loader)  + data)
        print("Testing Accuracy(%):",test_acc, "test loss:",test_loss.item())
        # writer.add_graph(testmodel,images)
